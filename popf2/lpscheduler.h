...
protected:
    ...
    typedef set<const Constraint*, ConstraintPtrLT> ConstraintSet;//根据ID大小从小到大排列
    ...
    public:
         ...
         */
        double postLastEffectValue;//记录常量值
         ...
         */
        int lastEffectValueVariable;//lp变量，记录上一次改变后的值，若为-1，该变量为常数
         ...
         */
        int lastEffectTimestampVariable;//记录上一个动作作用的时间变量，若为-1，说明该变量未曾用到
         ...
         */
        int activeGradientCount;//记录对该变量有连续数值效果的动作个数
...
private:
    const int arrSize;
    list<int> Q;
    bool * qSet;//表征Q中对应元素是否存在
public:
    bool * UB;
    bool * LB;
    bool * UBP;
    bool * LBP;
    int * NEW;//记录每个元素对应的配对步骤，例如：NEW[from] = to;
