...
//由TIL的特有添加/删除效果，更新文字的生命周期actionTSBounds
void TemporalAnalysis::processTILDeadlines()
{
...

    {
        ...
        map<int, double> literalAppears;//即不在初始状态中出现，也没有动作添加的文字，及其出现的时间
        map<int, double> literalDisappearsForGood;//TIL添加效果删除，删除效果加入的文字
        ...
        for (int tilID = 0; tilID < tilCount; ++tilID) {
            ...
            //由TIL删除效果确定每个被删除的文字是否只由TIL添加，若是，确定其生命周期windows[litID] = [x, tilItr->duration]
            {
                ...
                for (; effItr != effEnd; ++effItr) {
                    ...
                    if (wItr.second) {//插入成功，确定只被TIL添加的fact
                    ...
                    if (wItr.first->second) {//该文字只被TIL添加，确定它的生命周期windows[litID]
                        ...
                        //确定窗口开始时间
                        if (laItr.second) {//插入成功
                            if (dest.empty()) {//没有窗口，考虑是否在初始状态出现
                        ...
                        if (startOfWindow != DBL_MAX) {//构造窗口[startOfWindow, tilItr->duration]
                            if (debug) {
            ...
            //由TIL的添加效果确定文字出现的时候，填充lastAppearance
            {
                ...
                for (; effItr != effEnd; ++effItr) {
                    const int litID = (*effItr)->getStateID();
                    const pair<map<int, bool>::iterator, bool> wItr = windowable.insert(make_pair(litID, true));
                    if (wItr.second) {//插入成功，确定只被TIL添加的文字
                        const list<pair<int, VAL::time_spec> > & etaList = RPGBuilder::getEffectsToActions(litID);
                        list<pair<int, VAL::time_spec> >::const_iterator etItr = etaList.begin();
                        const list<pair<int, VAL::time_spec> >::const_iterator etEnd = etaList.end();
        ...
        //对以TIL特有文字为前提的动作，更新其有效时间范围actionTSBounds下界
        {
        ...
        //对以TIL特有删除文字为前提的动作，更新其有效时间范围actionTSBounds上界
        {
        ...
        //若窗口非空，更新lastAppearance的窗口上届为最大DBL_MAX
        {
...
//根据effectsToActions确定文字目标的实现时间上界，若dest对应的值更大则替换
...
//检查动作自身的生命周期上下界关系是否正常，以及开始，持续时间和结束时间是否正常
...
/* 参数：（动作的结束效果，以上一项参数为前提的动作，目标，第一项参数的子集：可被忽略的效果集合）
 * 返回值：假 - 结束效果会破坏动作前提或目标
 *      : 真 - 结束效果和动作前提、目标之间没有交集 
 * /
...
//填充参数，文字集合：以该文字为前提的动作要么可以直接删除它，要么全程需要但在动作结束时删除。则可被认为是安全压缩的
void TemporalAnalysis::recogniseHoldThroughoutDeleteAtEndIdiom(LiteralSet & factsIdentified)
{
    ...
    // In this case, moving the end delete to the start and scrubbing（擦洗，取消） the invariant is not equivalent.
    ...
    for (int lit = 0; lit < litCount; ++lit) {
        ...
        vector<set<int> > startEndDelete(3);//收集删除效果（前提条件）对应的动作（开始，结束，假动作）
        ...
        if (!startEndDelete[2].empty()) {//假动作不为空
            continue;
        }
        
        if (startEndDelete[1].empty()) {//结束删除动作为空
            //never end deleted, so no concerns
            continue;
        }
        
        vector<set<int> > actionsWithFactAsAStartInvEndCondition(3);//收集前提条件对应的动作（开始，中间，结束）
        ...
        set<int> onlyAsEnd;//结束前题对应的动作中删除不变前提对应的动作
        ...
        set<int> onlyAsAStart;//开始前题对应的动作中删除不变前提对应的动作
        ...
        set<int> onlyAsStartButNotThenDeleted;//纯开始前提对应的动作中排除开始删除效果的动作
...
vector<bool> TemporalAnalysis::startEndSkip;//能否直接越过中间步骤跳到结尾

/*
    有下列情况之一的即可认为不可安全压缩：
        结束数值效果、非开始数值条件（不变数值、结束数值前提、线性连续效果）、
        不变前提不包含结束前提、
        结束删除效果可能删除需要的前提、
        结束添加的效果可能与所需的正前提冲突
    */
