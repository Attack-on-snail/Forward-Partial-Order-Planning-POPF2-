...
//对新插入的节点u，计算0、u、与u配对的点（持续动作）或者u之前发生的点，之间的边权值，判断是否出现负环，并更新权值
//参数(LP队列，边e，0->e距离, e->0距离, 配对关系,  事件, 缓存边)
                 ...
                 map<int, IncomingAndOutgoing > & temporaryEdges)
{
    ...
    Q.visit(e.from, e.to);////对新插入的点做记录
    assert(e.from == -1 || events[e.from]);//不能凭空出现
    ...
    if (e.implicit) {
        ...
        } else {//若这条边未在temporaryEdges中出现，插入起点和终点，并添加对应的后继者（终点）和先驱者（起点）
        ...
        while (!Q.empty()) {
            ...
            if (Q.UB[u]) {
                ...
                if (d0u != DBL_MAX) {
                    if (bfDebug) {//表明这条边的端点的来源（TIL、动作开始、结束、0点）
                        if (u >= 0) {
                    ...
                    if (u >= 0) {//从u出去的边
                        ...
                        if (v >= 0) {  // and this has to be a sensible value, i.e. u isn't a TIL or instantaneous action
                            ...
                            //0 ---> u ----> v 和 0 ----> v做差比较，更新0 --> v的权值并判断是否异常
                            if (d0uplusWuv < (d0v - HALFEPSILON)) {
                                d0v = d0uplusWuv;
                                if (bfDebug) cout << "\t\td(zero-to-" << v << ") now = " << d0v << "\n";
                                if (d0v + distToZero[v] < -HALFEPSILON) {//出现负环
                        ...
                        //若u之前有步骤v要实现，判断0、u、v三者之间是否出现负环，更新边权值
...
private:
    ...
    vector<int> pairWith;//TIL：-2，非时间：-3
    vector<FFEvent*> eventsWithFakes;
    map<int, IncomingAndOutgoing> temporaryEdges;//缓存所有出现过的边，记录每个点之前/之后的点
                         ...
                         const bool & setObjectiveToMetric) : cd(0)
{
    ...
    bool paranoia = (Globals::paranoidScheduling || Globals::profileScheduling);//paranoia:妄想症
    ...
    vector<int> fanIn(tsVarCount, 0);//每一步之前的步骤，并且未访问的个数

    /* The steps that necessarily follow each step in the plan */
    vector<map<int, bool> > fanOut(tsVarCount);//每一步之后的步骤
     ...
     */
    vector<map<int, ConstraintSet > > invariantsThisStepStartsOnVariableI(tsVarCount);//与从该步开始的不变量v有关的约束
     ...
     */
    vector<set<int> > activeAncestorsOfStep(tsVarCount);//记录该步之前已经开始但未结束的动作，以确定受影响的不变量
     ...
     */
    vector<bool> correspondEndHasBeenInserted(tsVarCount,false);//与该步配对的结束动作已经调度完毕
    
    /* 必须在开始但未完成的动作结束之前发生的动作
    ...
    list<int> openList;//该步之前没有其它步骤要完成
    int imaginaryEnds = 0;//计划中持续性的结束步骤总数
    ...
    for (int i = 0; i < numVars; ++i) {
        finalNumericVars[i] = FluentTracking(initialValues[i]);//常值
...
void LPScheduler::initialise()
{
    ...
    for (int a = 0; a < actCount; ++a) {
        ...
        if (!RPGBuilder::rogueActions[a]) {
            interesting[a] = vector<InterestingMap>(3);//定义了3个元素
            ...
            if (discretisation) {
                ...
                //填充gradientEffects以及interesting
                bool allMetricTracking = true;
                {
                    //<变量ID，变量表达式>，填充gradientEffects以及interesting
            ...
            //根据数值效果填充instantEffects[a]以及interesting[a][0/2]的效果和前提（开始/结束）
            {
            ...
            //根据数值前提填充constraints[a]以及interesting[a][0/1/2]的前提（开始/中间/结束）
            {
            ...
            //考虑持续时间中的数值变量，填充interesting[a][0]的前提
            ...
            //考虑条件效果，填充pointsThatWouldBeMutexWithOptimisationTILs
            {
...
bool LPScheduler::isSolution(const MinimalState & state, list<FFEvent> & header, list<FFEvent> & now)
{
    ...
    /* 如果不使用线性规划，就判断数值条件是否满足问题中的metric，如满足：赋予plan最早的开始时间，否则返回false */
    if (!lp) {
...
//根据输入的当前规划和包含的时间约束，创造一个结构表示其中包含的时间约束以及开始动作序列（开始未结束）
...
//要不结束正在执行的动作（最后一个参数），要不执行新动作（第3个参数）。向STN中添加新边，并判断是否需要调用LP
                                       ...
                                       const TemporalConstraints * const cons, const int & stepID)
{
    ...
    const int mustComeBeforeOpenEnds = MinimalState::getTransformer()->stepThatMustPrecedeUnfinishedActions(cons);//-1
        
    double TILupbo = MinimalState::getTransformer()->latestTimePointOfActionsStartedHere(nextTIL);//DBL_MAX

    ChildData * toReturn = 0;

    //stepID已经存在，说明该步是需要结束该动作，确定其调度时间区间[lpMinTimestamp, lpMaxTimestamp],并向“STN”中加入新边
    if (startGap > stepID) { // if we're only triggering an existing action within the plan
        ...
        //若该结束动作没有引入与时间有关的效果/前提，不需要LP，否则需要（即若不涉及到连续效果，没必要利用LP求解）
        ...
        //若该动作必须在所有未完成动作之前发生，插入边endLocation -> p1
        if (mustComeBeforeOpenEnds != -1) {
            ...
            list<StartEvent>::iterator seqFillItr = seq.begin();//未完成动作
        ...
        {// now see if any new edges are in cons that weren't present before
            ...
            //比较在该步之前必须要实现的动作/事件，根据时间关系，插入新边（eventsBeforeEndAction->endLocation）
            if (eventsBeforeEndAction) {
                ...
                while (oldCbf != oldCbfEnd && newCbf != newCbfEnd) {
                    if (newCbf->first < oldCbf->first) {//事件发生更早
            ...
            //添加新边（endLocation-> list<StartEvent>）
            {
...
bool ChildData::propagateNewEdges()
{
    ...
    if (checkSanity) sanityCheck();//合理性检验
    ...
    if (floydDoubleCheck) {
        ...
        //根据getActionTSBounds()设置边上的权值，0 --> 点，最大值，反过来是最小值的负数
        for (int m = 1; m < mSize; ++m) {
            matrix[m][0] = 0.0;
            if (!eventsWithFakes[m-1]) continue;
            if (eventsWithFakes[m-1]->time_spec == VAL::E_AT_START) {
        ...
        for (int m = 1; m < mSize; ++m) {
            ...
            if (pairWith[m-1] >= 0) {//正常动作，动作开始 ----> 动作结束，权值为最小值的负数
                if (eventsWithFakes[m-1]->time_spec == VAL::E_AT_START) {
