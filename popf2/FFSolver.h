...
struct StartEvent {
    ...
    int stepID;
    double advancingDuration;//动作剩余的持续时间？
    double minDuration;
    double maxDuration;
    double elapsed;
    double minAdvance;
...
public:
    FakeFFEvent(StartEvent * const e, instantiatedOp * a, const int & pw, const double & dMin, const double &
                dMax)
            : F FEvent(a, pw, dMin, dMax), toUpdate(e) {
...
public:
    ...
    map<int, list<list<StartEvent>::iterator > > entriesForAction;//收集未竞动作<动作ID, 迭代器列表>
