...
void RPGBuilder::initialise()
{
    ...
    SimpleEvaluator::setInitialState(); //从问题中读取初始状态

    /* 操作实例化，仅限于操作名和参数 */
    for (operator_list::const_iterator os = current_analysis->the_domain->ops->begin();
         os != current_analysis->the_domain->ops->end(); ++os) {
        if (RPGdebug  || SHowDetail) cout << (*os)->name->getName() << endl;
        instantiatedOp::instantiate(*os, current_analysis->the_problem, *theTC);
        if (RPGdebug  || SHowDetail) cout << instantiatedOp::howMany() << " so far"<<endl;
    };
    if ((RPGdebug && Globals::globalVerbosity & 65536)) cout << instantiatedOp::howMany() << endl;
    if ((RPGdebug && Globals::globalVerbosity & 65536)) instantiatedOp::writeAll(cout);

    /* 谓词实例化并过滤无意义的词之后得到Literal . 之后可以得到问题中实例化的函数：PNE*/
    {
        int fpass = 1;
        int numBefore;
        do {
            if (RPGdebug  || SHowDetail) cout << "\nCollecting and filter, pass " << fpass << endl;
            instantiatedOp::createAllLiterals(current_analysis->the_problem, theTC);
            if ((RPGdebug && Globals::globalVerbosity & 65536)  || SHowDetail) instantiatedOp::writeAllLiterals(cout);
            cout<<endl;//强制刷新缓冲区，输出Debug数据

            numBefore = instantiatedOp::howMany();
            if (RPGdebug  || SHowDetail) {
                cout << "\tNumber of operators before filtering: " << instantiatedOp::howMany() << endl;
            }
            instantiatedOp::filterOps(theTC);

            if (RPGdebug || SHowDetail) {
                cout << "\tNumber of operators after filtering: " << instantiatedOp::howMany() << endl;
            }
            ++fpass;
        } while (instantiatedOp::howMany() < numBefore);
    }
    if ((RPGdebug && Globals::globalVerbosity & 65536)  || SHowDetail) instantiatedOp::writeAllPNEs(cout);
    cout<<endl;//强制刷新缓冲区，输出Debug数据
    ...
    instantiatedOp::assignStateIDsToNonStaticLiteralsAndPNEs();//为文字和PNE编号，从0开始
    
    if (RPGdebug  || SHowDetail) cout << "\nCaching action-literal dependencies"<<endl;
    ...
    //填充TimedPrecEffCollector类下的谓词/PNE名字和它的参数个数
    ...
    /* 由时间初始化文字填充数据：timedInitialLiterals、timedInitialLiteralsVector、effectsToActions、negativeEffectsToActions */
    {
        ...
        for (int i = 0; tilItr != tilEnd; ++tilItr, ++i) {
...

            {
                ...
                for (; elItr != elEnd; ++elItr) {
                    if(SHowDetail) {(*elItr)->write(cout); cout<<endl;}
    ...
    /* 填充非静态文字：literals */
    {
        ...
        for (; lsItr != lsEnd; ++lsItr) {
            ...
            if (i != -1) {//非静态文字
                literals[i] = *lsItr;
                if(SHowDetail) cout <<"\n"<< "Literal " << i << " - " << *(*lsItr) << " with global ID " << (*lsItr)->getGlobalID() << endl;
            }
        }
    }
    ...
    {
        if (PNEdebug  || SHowDetail) cout << "PNEs in RPG instantiation:"<<endl;
        ...
        for (; pneItr != pneEnd; ++pneItr) {
            ...
            if (PNEdebug  || SHowDetail) cout << *(*pneItr) << " with state ID " << sID << " and global ID " << (*pneItr)->getGlobalID() << endl;
        }
        if (PNEdebug  || SHowDetail) cout << "PNEs in operators:"<< endl;
    }
    ...
    /* 填充操作的前提、效果，时间表达式，以及不满足的前提条件数目
     * 填充数据：instantiatedOps、realRogueActions（rogueActions就是realRogueActions的常引用）
     */
    for (; opsItr != opsEnd; ++opsItr) {
        ...
        realRogueActions[operatorID] = checkIfRogue(c);//rogue：无赖，异常的，检查动作的必要元素是否都存在

        const bool rogueDebug = false;

        if (RPGdebug  || SHowDetail) cout << "Operator " << operatorID << " - " << *currOp << endl;

        if (rogueActions[operatorID]) {//rogueActions就是realRogueActions的常引用
            if (RPGdebug || SHowDetail) cout << "Rogue action, skipping"<<endl;
            ...
            if (rogueDebug || SHowDetail) cout << "Operator " << operatorID << " is a rogue"<<endl;
            #ifdef ENABLE_DEBUGGING_HOOKS
            ...
            /* 填充动作效果 */
            {
                /* 填充动作的开始的添加效果： actionsToStartEffects，effectsToActions */
                {
                    ...
                    if (RPGdebug  || SHowDetail) {cout << "Operator " << operatorID << " start adds:"; cout.flush();}


                    for (; effItr != effEnd; ++effItr) {
                        const int effID = (*effItr)->getStateID();
                        assert(effID >= 0);
                        if (RPGdebug  || SHowDetail) {cout << " " << *(*effItr) << " (" << effID << ")"; cout.flush();}
                        currEffectsList.push_back(*effItr);
                        effectsToActions[effID].push_back(pair<int, VAL::time_spec>(operatorID, VAL::E_AT_START));
                    }
                    if (RPGdebug  || SHowDetail) cout << endl;

                }

                /* 填充动作的开始的删除效果： actionsToStartNegativeEffects，negativeEffectsToActions */
                {
                    ...
                    if (RPGdebug  || SHowDetail) {cout << "Operator " << operatorID << " start deletes:"; cout.flush();}
                ...
                /* 填充动作结束的添加效果： actionsToEndEffects， effectsToActions */
                {
                    ...
                    if (RPGdebug  || SHowDetail) {cout << "Operator " << operatorID << " end adds:"; cout.flush();}
                ...
                /* 填充动作结束的删除效果： actionsToEndNegativeEffects， negativeEffectsToActions */
                {
                    ...
                    if (RPGdebug  || SHowDetail) {cout << "Operator " << operatorID << " end deletes:"; cout.flush();}
                ...
                /* 填充动作开始、结束的数值效果： actionsToStartNumericEffects，actionsToEndNumericEffects */
                {
                    actionsToStartNumericEffects[operatorID] = c.startNumericEff;

                    if (RPGdebug  || SHowDetail) {
                        cout << "Operator " << operatorID << " start numeric effects:"<<endl;
                    ...
                    if (RPGdebug  || SHowDetail) {
                        cout << "Operator " << operatorID << " end numeric effects:"<<endl;
            ...
            /* 填充动作前提 */
            {
                if (RPGdebug  || SHowDetail) cout << "Operator requires at start:"<<endl;

                /* 填充动作开始的前提条件： actionsToStartPreconditions， preconditionsToActions，initialUnsatisfiedStartPreconditions */
                {
                    ...
                    for (; precItr != precEnd; ++precItr) {
                        ...
                        if (precID >= 0) {
                            if (RPGdebug  || SHowDetail) {cout << " " << *(*precItr) << " (" << precID << ")"; cout.flush();}
                ...
                /* 填充动作开始的负前提： actionsToStartNegativePreconditions， negativePreconditionsToActions */
                {
                ...
                if (RPGdebug  || SHowDetail) {cout << "Operator requires as an invariant:"; cout.flush();}

                /* 填充动作的持续条件： actionsToInvariants， preconditionsToActions，initialUnsatisfiedInvariants */
                {
                ...
                /* 填充动作的负持续条件： actionsToNegativeInvariants， negativePreconditionsToActions */
                {
                ...
                if (RPGdebug  || SHowDetail) {cout << "Operator requires at end:"; cout.flush();}

                /* 填充动作结束的前提条件： actionsToEndPreconditions， preconditionsToActions，initialUnsatisfiedEndPreconditions */
                {
                ...
                /* 填充动作结束的负前提： actionsToEndNegativePreconditions， negativePreconditionsToActions，preconditionlessActions */
                {
                    ...
                    if (currPreconditionsList.empty() && actionsToEndPreconditions[operatorID].empty()) {
                        if (RPGdebug  || SHowDetail) cout << "Operator is preconditionless at the end"<<endl;
                        preconditionlessActions.push_back(pair<int, VAL::time_spec>(operatorID, VAL::E_AT_END));
                    }
                }

                /* 填充动作的数值前提： actionsToStartNumericPreconditions， actionsToNumericInvariants，actionsToEndNumericPreconditions */
                {
            ...
            if (RPGdebug  || SHowDetail) {
                cout << "Start negative effects:"<<endl;
                {
                    ...
                    for (; llItr != llEnd; ++llItr) {
                        cout << "\t" << *(*llItr) << " (" << (*llItr)->getStateID() << ")"<<endl;
                    }

                }
            }

            /* 从原有的开始前提中找到自身不能满足的不变前提，冠名processed
             * now do the tRPG processing:
             - collapse invariants into start actions, collapse ... into ... :陷入
            ...
            */
            for (int pass = 0; pass < 2; ++pass) {
                ...
                LiteralSet oldStartEffects;//收集最初的开始添加/删除效果
                {
                    ...
                    for (; effItr != effEnd; ++effItr) {
                        oldStartEffects.insert(*effItr);//根据ID从小到大排列
                    }
                }

                /* 找到那些在该动作中不被自身满足的不变量条件，存入集合 newStartPrecs */
                {
                    ...
                    for (; precItr != precEnd; ++precItr) {
                        if ((*precItr)->getStateID() >= 0) {
                            if (oldStartEffects.find(*precItr) == notFound) {
                                if (RPGdebug  || SHowDetail) {
                                    if (pass) {
                                        cout << "Negative invariant " << *(*precItr) << " is not met by start effects"<<endl;
                                    } else {
                                        cout << "Invariant " << *(*precItr) << " is not met by start effects"<<endl;
                                    }
                                }
                ...
                if (RPGdebug  || SHowDetail) {
                    if (pass) {
                        cout << "Processed start negative preconditions:"<<endl;
                    } else {
                        cout << "Processed start preconditions:"<<endl;
                    }
                ...
                /* 更新 initialUnsatisfiedProcessedStartPreconditions，preconditionlessActions */
                if (!pass) {
                ...
                /* 更新 开始 processedNegativePreconditionsToActions，processedPreconditionsToActions */
                {
                ...
                /* 更新 结束 processedNegativePreconditionsToActions，processedPreconditionsToActions */
                {
    ...
    /* 更新集合preconditionsToActions */
    oneShotInferForTILs(); // this is done here, as it adds extra invariants to actions

    if (doTemporalAnalysis) {
        TemporalAnalysis::processTILDeadlines();//由TIL的特有添加/删除效果，更新文字的生命周期actionTSBounds
        TemporalAnalysis::findGoalDeadlines(literalGoals, literalGoalDeadlines);//根据effectsToActions确定文字目标的实现时间上界，若第二个参数对应的值更大则替换
    } else {
        TemporalAnalysis::dummyDeadlineAnalysis();//构造默认的分段动作时间范围（0，DBL_MAX）
    }

    findStaticLiterals();//填充staticLiterals

    if (RPGBuilder::readPropositionGroups) {
        RPGBuilder::readPropositionGroupsFile();
    }

    postFilterUnreachableActions();//初步过滤不可实现的动作
    //考虑两种情况删除该动作：1-动作前提是静态的且为false；2-动作前提非静态但是没有其他动作实现该前提

    pruneStaticPreconditions();//删除静态前提
    ...
    handleNumericInvariants();
     ...
     /* 遍历无条件的动作，筛选数值前提的动作 onlyNumericPreconditionActions */
    {
    ...
    if (doTemporalAnalysis) {
        TemporalAnalysis::findActionTimestampLowerBounds();//更新动作的开始/结束事件下界
        postFilterIrrelevantActions();
    } else {
        globalHeuristic = generateRPGHeuristic();//构造new RPGHeuristic()
    }
...
//动作实现需要在这些步骤（填充第三个参数）之前/之后
bool RPGBuilder::stepNeedsToHaveFinished(const ActionSegment & act, const MinimalState & theState, set<int> & toBeNonMutex)
{
    const bool debug = false;

    const int actID = (act.first ? act.first->getID() : -1);//-1说明该动作是TIL

    list<Literal*> * willDelete = 0;//删除效果
    list<Literal*> * willAdd = 0;//添加效果

    list<Literal*> * needs = 0;//正前提
    list<Literal*> * negativeNeeds = 0;//负前提

    unsigned int invariantExemption = INT_MAX;//跳过的不变量数目
    ...
    //可以安全跳过持续阶段，往toBeNonMutex加入当前状态下（正的）要删除的步骤id
    if (willDelete) {
        list<Literal*>::iterator fItr = willDelete->begin();
        const list<Literal*>::iterator fEnd = willDelete->end();
    ...
    //可以安全跳过持续阶段，往toBeNonMutex加入当前状态下（负的）要添加的步骤id
    if (willAdd) {
    ...
    //往toBeNonMutex加入当前状态下（正的）动作正前提对应的步骤id
    if (needs) {
    ...
    //往toBeNonMutex加入当前状态下（负的）动作负前提对应的步骤id
    if (negativeNeeds) {
...
double RPGBuilder::DurationExpr::minOf(const vector<double> & minFluents, const vector<double> & maxFluents)
{
    ...
    for (int i = 0; i < lim; ++i) {
        ...
        if (currW < 0.0) {
            toReturn += currW * getValue(maxFluents,variables[i]);//maxFluents[ variables[i]  ]
        } else {
...
//a:动作ID，div：durationExpressions的序号,minFluents：不同变量的最小取值，maxFluents：不同变量的最大取值，
//非持续动作的持续时间默认为0.001,持续动作的持续时间为<最小，最大>
...
//收集动作的开始/结束效果，分别存入add,del,numeric
...
#else
    pair<list<double>, list<int> > result;//权重，变量（编号）
#endif
