...
#define UNSAFETOSKIP false
    ...
    StepAndBeforeOrAfter availableFrom;//正的命题可以从该步之前/之后开始可用
    map<StepAndBeforeOrAfter, bool> deletableFrom;//命题可以从该步之前/之后开始删除
    ...
    PropositionAnnotation(const bool polarity = true) {
        if (polarity) {
            negativeAvailableFrom.setToNever();//在never步之后
        } else {
     ...
     *  第一个参数：命题在该步之前/之后可用
     *  第二个参数：若命题在该步epsilon之后可用，则为true
     *  第三个参数：若命题在指定时间可用则为true，若该命题的否在指定时间可用则为false
     */
    ...
    */
    ...
    void markAsDeleted(const StepAndBeforeOrAfter & step) {
        if (!availableFrom.never()) {//不是never
...
public:
    ...
    #else
    StateFacts first;//正的，添加
    StateFacts retired;//负的事实，删除
    #endif

    vector<double> secondMin;//和问题模型中的评价标准metric有关
