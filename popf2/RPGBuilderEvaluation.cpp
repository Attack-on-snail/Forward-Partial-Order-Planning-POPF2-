 ...
 *  确定目前该数值条件成立的最早时间（该数值条件左右值成立的最晚时间）
...
struct RPGRegress {
     ...
     */
    map<int, pair<double,double> > numericValueGreaterThan;//在当前变量层，变量左值需要大于等于pair的第一个元素，TIL的权值为pair第二个元素
    
    /** Request a numeric precondition in this layer.在当前层variable层插入数值条件
     *
...
#ifdef POPF3ANALYSIS
//当前层（时间）由achiever耗费costs完成。是否耗费代价用costFree描述
...
struct ActionAndHowManyTimes {
     ...
     */    
    bool gradientNeedsStarting;//要么该动作首次出现，要么其“梯度耗尽”且需要再次使用，都要求该动作开始
...
struct FactLayerEntry {  
    pair<set<int>, set<int> > * endOfJustApplied;//分别是开始动作、结束动作
     ...
     */
    list<int> first;//当前事实层中新增的命题fact
    ...
    *  in <code>RPGBuilder::literals</code>.
    */
    set<int> firstRepeated;//当前事实层中代价更低的fact
     ...
     */
    list<pair<int, int> > TILs;//<TIL编号，TIL添加的事实ID>
...
public:
    ...
    struct RecordedAssignment {   
        ...
        const ActionAndHowManyTimes * act;//具有数值赋值效果的动作
         ...
         *  should be used.
         */
        bool maximiseEffect;//若为真，应该给该赋值变量最大值，否则最小值
    ...
    protected:
        ...
        /** @brief The layer should only be used to dictate（规定，指示） input values to effects.
         *
         ...
         */
        bool onlyUseLayerForEffectMagnitudes;//若为真，该fluent层只是为了给动作的RHS赋值
         ...
         */
        vector<double> internalValues;//当前层中变量的最大值
        ...
        /** @brief If the recorded value of a variable was due to an assignment effect, note this here.
          ...
          */         
        vector<RecordedAssignment*> assignmentEffectThatGaveThisValue;//用于解提取
    ...
    public:
         ...
         *  the gradient-induced change between the previous fact layer, and this one.
         *
...
protected:
     ...
     *  尝试添加新fluent层（startAt->first + timeDifference），若该层首次出现，补充相关信息，否则返回之前层的iterator
     * 
    ...
    */
     ...
     */
    map<double, set<int> > preconditionsBecomingTrueAtTime;//因为激活的梯度而成立的前提条件以及成立的时间
    ...
    */
    ...
    /** @brief Common initialisation code when recording effects. 记录效果时的通用初始化过程
     *  效果层若未创建，尝试创建；否则判断效果层与之前的fluent层时间差小于EPSILON/2，小于则正常
     *  This function ensures that: 该函数保证以下内容：
     *  - a fluent layer has been created for the specified time，变量层在之前（参数-EPSILON）已经定义
    ...
    void aboutToRecordAnEffect(const double & effectAppearsInLayer) {
        if (effectsAffectThisLayer == layers.end()) {//effectAppearsInLayer该层未在已扩展的TRPG中出现
            effectInputs = layers.find(effectAppearsInLayer - EPSILON);//寻找于该层直接相关的fluent层
            #ifndef NDEBUG
            ...
            //之前的变量层（参数-EPSILON）已经定义，但是effectsAffectThisLayer还未定义，尝试创建
            effectsAffectThisLayer = addLayer(effectInputs, EPSILON);//尝试创建新层（effectInputs->first + EPSILON，即当前层（参数））
...
public:
    ...
    //属于数值层，返回下一层的时间（若不需要revisit是动作的的时间，否则是动作的时间-EPSILON）以及下一层是否在当前层时间epsilon之后
    //若layerWithEffectsToBeRevisited存在需要修改的效果，立即进行修改，返回make_pair(layerWithEffectsToBeRevisited->first, true);
    //否则若preconditionsBecomingTrueAtTime存在元素，返回make_pair(ptItr->first - EPSILON, ((ptItr->first - 2 * EPSILON) < 0.000001));
    ...
    pair<double,bool> nextLayerWouldBeAt(bool & isToRevisitEffects, bool & isDueToGradients) { 
        ...
        if (layerWithEffectsToBeRevisited != layers.end()) {
            ...
            return make_pair(layerWithEffectsToBeRevisited->first, true);//数值效果在之前层EPSILON之后
        }
        ...
        return make_pair(ptItr->first - EPSILON, ((ptItr->first - 2 * EPSILON) < 0.000001)); //数值前提在下一层EPSILON之前，是否在上一层EPSILON之后
    }

    /** @brief Create a dummy fluent layer at the time given.
     *  （当前层）|---|（下一层）之间时间差若大于EPSILON/2，需要确保输入的fluent层存在，否则输入的fluent存在
    ...
    //根据需要revisit的集合，更新mostRecentInstantaneousEffects、mostRecentIntegratedEffects、mostRecentGradientEffects
    void recordEffectsThatAreNowToBeRevisited(map<double, FactLayerEntry, EpsilonComp > & factLayers) {
        ...
        //遍历revisit中的元素，收集以其为效果的动作actionsThatHaveHadThisEffect，存入mostRecentInstantaneousEffects
        {
        ...
        //遍历revisit2中的元素，收集以其为效果的动作actionsThatHaveHadThisIntegratedEffect，存入mostRecentIntegratedEffects
        //遍历revisit2中的�����������素，收集以其为效果的动作actionsThatHaveHadThisGradientEffect，存入mostRecentGradientEffects
        {
            for (int pass = 0; pass < 2; ++pass) {
     ...
     *  填充layerAtWhichPreconditionBecomesTrue，尝试创建时间为参数的层
    ...
    void recordConsequencesOfActiveGradients(const double & newFactsAppearsInLayer) {
        ...
        set<int>::const_iterator fItr = gradientsLeadToTheseFactsNowBeingTrue.begin();//preconditionsBecomingTrueAtTime
        ...
        aboutToRecordAnEffect(newFactsAppearsInLayer);//尝试创建该层
    }
     ...
     *  完成三件事：尝试创建effectAppearsInLayer层，记录该层被实现的动作actionsThatHaveHadThisIntegratedEffect，记录最后影响该效果的动作mostRecentIntegratedEffects
     ...
     *  完成四件事：尝试创建effectAppearsInLayer层，记录该层被实现的动作actionsThatHaveHadThisGradientEffect，记录最后影响该效果的动作mostRecentGradientEffects，
     *  并将该梯度效果最终耗尽的信息记录在factLayers
     ...
     *  action在effectAppearsInLayer实现效果effID
     * 完成三件事：尝试创建effectAppearsInLayer层，记录该层被实现的动作actionsThatHaveHadThisEffect，记录最后影响该效果的动作mostRecentInstantaneousEffects
     * @param action                Details on the action, including how many times it can be applied
    ...
    void recordInstantaneousNumericEffect(const ActionAndHowManyTimes & action, const double & effectAppearsInLayer, const int & effID) {
        ...
        aboutToRecordAnEffect(effectAppearsInLayer);//若参数层未定义，尝试创建
        
        //若不存在效果为函数第三个参数的动作，考虑effID的RPG数值效果，若变量ID大于0，该变量需要revisit
    ...
    */
    ...
    /** @brief Request that a numeric goal be achieved, during solution extraction.考虑数值目标，填充数值条件
     *
    ...
    void requestGoal(const int & pre, const double & achievedAt,
        ...
        addTo.requestNumericThreshold(currPre.LHSVariable, threshold, DBL_MAX);//（数值目标的左值，数值目标的右值，TIL的权值）在当前层插入数值条件
    }
    ...
    void recordSideEffects(const int & act, const VAL::time_spec & ts, const double & actLayer) {
        LayerMap::const_iterator rollback = layers.end();//变量层
        --rollback;
...
struct NextFactLayer {
    ...
    pair<double,bool> timestamp;//<时间值，是否在上一层EPSILON时间后>
...
struct DotDetails {
    ...
    list<string> factsToActions;//act -> fact
    ...
    //判断输入的字符串是否在actionLayerNodes中出现过，若出现过，返回TRUE
    ...
    //读取act -> fact关系，填充factsToActions：（开始）s -> fact; (结束-openend) ne -> fact; (结束) e -> fact
    ...
    //添加动作节点，填充actionLayerNodes、endActionsToStarts、factsToActions
    ...
    //为openEnd的动作添加动作层节点，填充actionLayerNodes、factsToActions
    ...
    //建立非数值fact -> act边，填充factsToActions（动作的前提）
    ...
    //建立数值fact -> act边，填充factsToActions（动作的前提）
...
public:
    ...
    Private(const bool & b,//deleteArrays
            vector<list<Literal*> > * atse,//actionsToStartEffects
            vector<list<Literal*> > * atee,//actionsToEndEffects
            vector<list<pair<int, VAL::time_spec> > > * eta,//effectsToActions
            vector<list<Literal*> > * atsne,//actionsToStartNegativeEffects
            vector<list<Literal*> > * atene,//actionsToEndNegativeEffects
            vector<list<pair<int, VAL::time_spec> > > * neta,//negativeEffectsToActions
            vector<list<pair<int, VAL::time_spec> > > * pta,//preconditionsToActions
            vector<list<Literal*> > * atsp,//actionsToStartPreconditions
            vector<list<Literal*> > * ati,//actionsToInvariants
            vector<list<Literal*> > * atep,//actionsToEndPreconditions
            vector<list<RPGBuilder::NumericEffect> > * atnuse,//actionsToNumericStartEffects
            vector<list<RPGBuilder::NumericEffect> > * atnuee,//actionsToNumericEndEffects
            vector<list<int> > * atrnuse,//actionsToRPGNumericStartEffects
            vector<list<int> > * atrnuee,//actionsToRPGNumericEndEffects
            vector<list<int> > * atnusp,//actionsToNumericStartPreconditions
            vector<list<int> > * atnui,//actionsToNumericInvariants
            vector<list<int> > * atnuep,//actionsToNumericEndPreconditions
            vector<list<int> > * atpnuep,//actionsToProcessedStartNumericPreconditions
            vector<int> * iusp,//initialUnsatisfiedStartPreconditions
            vector<int> * iuip,//initialUnsatisfiedInvariants
            vector<int> * iuep,//initialUnsatisfiedEndPreconditions
            vector<double> * ail,//achievedInLayer
            vector<double> * ailr,//achievedInLayerReset
            vector<pair<int, VAL::time_spec> > * ab,//achievedBy
            vector<pair<int, VAL::time_spec> > * abr,//achievedByReset
            vector<double> * nail,//numericAchievedInLayer
            vector<double> * nailr,//numericAchievedInLayerReset
            vector<ActionFluentModification*> * nab,//numericAchievedBy
            vector<ActionFluentModification*> * nabr,//numericAchievedByReset
            vector<int> * iunsp,//initialUnsatisfiedStartPreconditions
            vector<int> * iuni,//initialUnsatisfiedInvariants
            vector<int> * iunep,//initialUnsatisfiedEndPreconditions
            vector<RPGBuilder::RPGNumericPrecondition> * rnp,
            vector<RPGBuilder::RPGNumericEffect> * rne,
            vector<list<pair<int, VAL::time_spec> > > * ppta,//processedPreconditionsToActions
            vector<list<pair<int, VAL::time_spec> > > * nppta,//NegtiveProcessedPreconditionsToActions
            vector<list<Literal*> > * atpsp,//actionsToProcessedStartPreconditions
            vector<int> * iupsp,//initialUnsatisfiedProcessedStartPreconditions
            vector<int> * iupsnp,//initialUnsatisfiedProcessedStartNumericPreconditions
            list<pair<int, VAL::time_spec> > * pla,//preconditionlessActions
            list<pair<int, VAL::time_spec> > * onpa)//onlyNumericPreconditionActions
...
//            earliestNumericPrePOTimes = vector<double>(numericAchievedInLayer->size());
    ...
    {
    public:
        DotDetails dot;//要输出的PRG信息
        
        const MinimalState & startState;
        const list<StartEvent> * startEventQueue;
        
        vector<int> startPreconditionCounts;//每个动作不满足的前提数目
        ...
        list<pair<set<int>, set<int> > > setsToForbid;//收集刚刚应用完的事实层，便于后续回收
        
        map<double, FactLayerEntry, EpsilonComp > factLayers;//刚添加动作的新增事实
        FluentLayers fluentLayers;//刚添加动作的新增数值前提
        map<double, map<int, list<ActionFluentModification> >, EpsilonComp> fluentModifications;
        map<double, list<int>, EpsilonComp > endActionsAtTime;//肯定动作会在该时间结束，一般不变
         ...
         *  - the ends of actions which have been scheduled to appear,即endActionsAtTime
         *  - new facts due to TILs
         *  - the next point at which a numeric precondition becomes satisfied based on，即preconditionsBecomingTrueAtTime
        ...
        void nextThingToVisit(const double & currTS, NextFactLayer & next) {
            ...
            //若endActionsAtTime不为空，则下一层为动作层，且若其发生时间小于当前时间+EPSILON（0.01）,next.timestamp.second = true;
            const map<double, list<int>, EpsilonComp >::iterator eaItr = endActionsAtTime.begin();//endActionsAtTime是肯定会在当前时间结束的动作
            ...
            bool isToRecalculate = false;//是否在下一层重访效果
            bool isDueToGradients = false;//是否在下面函数返回的时间之后EPSILON应用梯度效果
            ...
            if (fluentLayersWantToRevisit.first != DBL_MAX) {
                if (fluentLayersWantToRevisit.second) {//在当前EPSILON之后
        ...
        vector<double> endActionSchedule;//动作结束时间的安排，可变动
        vector<double> openEndActionSchedule;

        const map<int, set<int> > & startedActions;
        int unsatisfiedGoals;
        int unappearedEnds;//insistUponEnds的个数
        bool tooExpensive;
        map<int, set<int> > insistUponEnds;//该动作无法安全跳过
        map<int, int> forbiddenStart;//<动作ID，禁止次数？>
...
#define MAKESTARTMDI(dest,a) MaxDependentInfo dest = a
    ...
    #ifdef POPF3ANALYSIS
    vector<list<CostedAchieverDetails> > achieverDetails;//详细记录每一层的实现层相关层、耗费代价信息
    ...
    vector<bool> numericIsTrueInState;//rpgNumericPreconditions
    ...
    list<pair<int, VAL::time_spec> > noLongerForbidden;//前提条件都得到满足的动作
...
//    static vector<double> earliestNumericPrePOTimes;
    ...
    /* 若实现factID有一个代价更低的动作，则更新(两种情况)：
     * 1 - 使用POPF3ANALYSIS，有cost-free的动作，则更新achieverDetails[factID]，若其中存储数据只有一个，返回2，否则返回1
     * 2 - 不使用POPF3ANALYSIS，若(*(achievedInLayer))[factID]是新的，更新其值为cTime,并将actID插入achievedBy，返回2
     * 否则返回0
     * */
    int proposeNewAchiever(const int & factID,
        ...
        //更新动作前提/效果中每个变量的actCosts
        for (int pPass = (openEnd ? 1 : 0); pPass < 2; ++pPass) {
            
            if (pPass == 1 && ts == VAL::E_AT_START) {//是openEnd且实现动作的阶段是AT_START，直接跳过
                break;
            }
            
            const list<Literal*> & pres = (pPass == 1 ? (*actionsToEndPreconditions)[actID] : (*actionsToProcessedStartPreconditions)[actID]);
            list<Literal*>::const_iterator pItr = pres.begin();//动作前提
            const list<Literal*>::const_iterator pEnd = pres.end();
            
            //找到不晚于当前层的实现动作前提层，更新每个数值变量的代价值actCosts为绝对值最大者
            for (; pItr != pEnd; ++pItr) {
            ...
            //非openEnd且动作为AT_END，actCosts加入动作开始效果中的每个变量代价
        ...
        //判断动作的开始/结束效果中每个变量的使用代价actCosts是否大于当前代价值currentCosts，若大于，costFree为False
        {
            //开始/结束动作每个变量的资源使用限制
        ...
        //找到不消耗代价的动作，插入achieverDetails[factID]并返回
        if (costFree) {
        ...
        // if we get this far it's a plausible achiever，首次获得factID的实现，返回2
        ...
        vector<double> admissibleCosts(achieverDetails[factID].back().costs);//已存在的最优factID实现代价
        
        //根据情况更新admissibleCosts，若有变动，anyImprovement = true
        for (int ul = 0; ul < ulCount; ++ul) {
    ...
    //根据动作涉及到的数值，填充actionsAffectedByFluent
    void populateActionFluentLookupTable() {
        ...
        //根据新增后的数值前提和结束前提，填充actionsAffectedByFluent
        for (int pass = 0; pass < 2; ++pass) {
        ...
        //根据数值开始效果和结束效果，填充actionsAffectedByFluent
        for (int pass = 0; pass < 2; ++pass) {
        ...
        //根据动作的持续时间，填充actionsAffectedByFluent
        for (int a = 0; a < actCount; ++a) {
            if (RPGBuilder::rogueActions[a]) continue;
        ...
        //根据动作的线性效果，填充actionsAffectedByFluent
    ...
    //整合效果：填充 integratedCTSEffectVar 和 integratedCTSEffectChange
    //不整合  ：填充 gradientCTSEffectVar 和 gradientCTSEffectChange
    void integrateContinuousEffects() {
        ...
        //若不使线性连续效果即刻化，非静态的RPG数值效果对应的变量ID存入cannotUseAsGradients
    ...
    BuildingPayload * spawnNewPayload(const MinimalState & theState, const list<StartEvent> * seq, const vector<double> & minTimestamps, list<ActionSegment> & haIn) {
        ...
        //根据toReturn的变量确定变量的最大取值，填充maxFluentTable
        {
    ...
    /** @brief Prime（准备好） the TRPG with the gradient effects of executing actions.开始动作不为空才有这项处理
     *
...
#else
    //根据系统设置的参数：是否微调RPG，来设定初始状态下各正负fact、数值条件、pne时间
    void recordFactLayerZero(BuildingPayload * const payload) {
        //计算每个变量的最大取值(secondMax/secondMin)*limit，存入currentCosts
        #ifdef POPF3ANALYSIS
        const vector<NumericAnalysis::NumericLimitDescriptor> & limits = NumericAnalysis::getGoalNumericUsageLimits();
        ...
        //修改初始状态每个fact的时间为0或者其他值（若修改，可能部分为其他值）
        if (RPGBuilder::modifiedRPG) {
            StateFacts::const_iterator stateItr = payload->startState.first.begin();
            const StateFacts::const_iterator stateEnd = payload->startState.first.end();

            for (; stateItr != stateEnd; ++stateItr) {
                const int factID = FACTA(stateItr);//stateItr->first
                double * achievedTS;
                #ifdef POPF3ANALYSIS
                achieverDetails[factID].push_back(CostedAchieverDetails(currentCosts,true));//-1层实现该层（t=0），不耗费代价,cost = currentCosts
                achievedTS = &(achieverDetails[factID].back().layer);//等于0
                #else
        ...
        //根据数值前提在初始状态中是否得到满足的情况确定numericIsTrueInState
        {
            const int loopLim = rpgNumericPreconditions->size();
    ...
    //统计当前TPRG下开始动作中不可以安全跳过的数目，填充payload->insistUponEnds、payload->unappearedEnds
    ...
    //过滤初始状态下就满足的目标/数值目标，更新payload->unsatisfiedGoals
    void seeWhatGoalsAreTrueToStartWith(BuildingPayload * const payload) {
        //寻找在初始状态下就成立的目标，减少目标不满足数量即payload->unsatisfiedGoals
        {
    ...
    //遍历TRPG中的开始动作startEventQueue，若动作还未调度或者调度时间大于下一层的时间，更新调度时间为下一层的时间
    ...
    //根据payload->propositionMustBeDeletedAddedAfter 和 earliestNumericPOTimes 确定受影响的动作是否应该开始/结束
    //更新layerEntry.endOfJustApplied、payload->forbiddenStart、payload->forbiddenEnd
    void addTemporalConstraintsFromActiveActions(BuildingPayload * const payload) {
        if (!RPGBuilder::modifiedRPG) {
            return;
        }

        //pair<double, double>分别是删除的时间、添加的时间
        ...
        //若之后出现了本应该禁止的文字，则以他为正效果/负效果的开始/结束动作应该立即结束（即被禁止开始/结束）
        //填充layerEntry.endOfJustApplied、payload->forbiddenStart、payload->forbiddenEnd、
        for (; constrItr != constrEnd; ++constrItr) {
            ...
            for (int pass = 0; pass < 2; ++pass) {
                ...
                if (!layerEntry.endOfJustApplied) {//没有刚刚应用完的层
                ...
                for (; effItr != effEnd; ++effItr) {
                    if (!RPGBuilder::rogueActions[effItr->first]) {
                        if (effItr->second == VAL::E_AT_START) {
                            if (dest->first.insert(effItr->first).second) {//若添加该文字的开始动作没有在endOfJustApplied中出现，添加进去
        ...
        //根据每个变量最早出现的时间值确定相应的事实层，然后根据actionsAffectedByFluent判断开始/结束动作是否在该层的
        //endOfJustApplied中出现，若没有，然它结束，并更新payload->forbiddenEnd、payload->forbiddenStart
    ...
    //根据第二个参数，确定删除动作的不变量的动作/TIL的发生时间
    //更新layerEntry.endOfJustApplied、payload->forbiddenStart、payload->forbiddenEnd
        ...
        for (; jaItr != jaEnd; ++jaItr) {
            ...
            if (insLayer > 0.0) {
                ...
                for (; liItr != liEnd; ++liItr) {
                    const int currActID = liItr->first;//justApplied的动作ID

                    //假定刚应用的动作为a，删除a的不变量条件动作为b，则应把b放在a之后，记录在affectedStarts、affectedEnds；
                    //删除a不变量的TIL为c，若c的时间小于当前状态的时间（TILoffset < stateTS，没看明白什么意思），直接报错
                    {
                        ...
                        for (; invItr != invEnd; ++invItr) {
                            const int litID = (*invItr)->getStateID();//刚应用动作a的不变量条件
                            list<pair<int, VAL::time_spec> > & affected = (*negativeEffectsToActions)[litID];//删除该不变量的动作b
                            list<pair<int, VAL::time_spec> >::iterator affItr = affected.begin();
                            const list<pair<int, VAL::time_spec> >::iterator affEnd = affected.end();
                            //b影响a，推迟它的发生时间到a的发生时间
                            ...
                            if (nextTIL < tilCount) {
                                for (int i = nextTIL; i < tilCount; ++i) {
                                    ...
                                    if (earliestTILtime < insLayer) {//上一个TIL在刚应用动作之前
                                        list<int>::iterator effItr = tilTemporaryNegativeEffects[i].begin();//删除该不变量的TIL
                                        const list<int>::iterator effEnd = tilTemporaryNegativeEffects[i].end();

                                        for (; effItr != effEnd; ++effItr) {
                                            if (*effItr == litID) {//删除该不变量
                    ...
                    //判断TIL是否删除a的结束前题并满足一定的关系（TILoffset < stateTS），寻找dead end
                    if (nextTIL < tilCount) {
                        ...
                        for (; precItr != precEnd; ++precItr) {
                            const int litID = (*precItr)->getStateID();//动作a的结束前题
                            for (int i = nextTIL; i < tilCount; ++i) {
                                const double earliestTILtime = tilTimes[i] - TILoffset;
                                ...
                                if (earliestTILtime < insLayer) {
                                    ...
                                    for (; effItr != effEnd; ++effItr) {
                                        if (*effItr == litID) {//TIL的删除效果删除动作a的结束前题
                                            TILoffset -= ((insLayer - earliestTILtime) + EPSILON);
                                            if (TILoffset < stateTS) {
                                                if (evaluateDebug) cout << "Dead end found: TIL definitely going to delete invariant of open action\n";
                                                return false;
                ...
                //若affectedStarts或affectedEnds不为空，
                //更新payload->factLayers[insLayer].endOfJustApplied、payload->forbiddenStart、payload->forbiddenEnd
                ...
                //若affectedEnds非空，获取其中各动作的最大持续时间，倒推动作可能的开始时间
                //若其所在层的endOfJustApplied不存在集合affectedEnds中的动作，更新payload->forbiddenStart
            ...
            //若刚应用完成动作的调度时间还未安排，给其安排成insLayer
            {
                list<pair<int, int> >::iterator liItr = jaItr->second.begin();
                const list<pair<int, int> >::iterator liEnd = jaItr->second.end();

                for (; liItr != liEnd; ++liItr) {
                    const int currActID = liItr->first;//刚应用完成的动作
    ...
    //主要是为每个处理后的前提、TIL特有删除效果设置deadlineAtTime为DBL_MAX、TIL删除时间
    void performTILInitialisation() {
        ...
        //根据处理后的前提条件，设置每个fact的期限deadlineAtTime为最大DBL_MAX
        {
        ...
        //根据TIL的添加/删除效果填充tilEffects、tilNegativeEffects、deadlineAtTime
    ...
    //设置earliest/latestStartAllowed、earliest/latestEndAllowed
    ...
    //将TIL的添加效果加入事实层，根据其删除效果更新以该效果为前提的动作的开始/结束时间上限
    void addTILsBeforeExpansion(const int & nextTIL, map<double, FactLayerEntry, EpsilonComp > & factLayers, const double & stateTS, const double & tilFrom) {
        ...
        //处理TIL的添加效果，若其不在初始状态中，填充factLayers[thisTS].TILs
        for (int i = nextTIL; i < tilCount; ++i) {
        ...
        //处理TIL的删除效果，更新以其为前提的动作的开始/结束时间，即latestStartAllowed、latestEndAllowed
        for (int i = nextTIL; i < tilCount; ++i) {
            ...
            for (; effItr != effEnd; ++effItr) {
                list<pair<int, VAL::time_spec> > & currList = (*preconditionsToActions)[*effItr];//以TIL删除效果为前提的动作
    ...
    //latestStartAllowed中动作a的开始/结束时间上限值-函数参数，若小于latestStartAllowed[a]的值，替换掉集合中的值
    void addBoundsFromTemporalAnalysis(const double & stateTS) {
        const int actCount = latestStartAllowed.size();

        for (int a = 0; a < actCount; ++a) {
            {
                double firmUpper = TemporalAnalysis::getActionTSBounds()[a][0].second;//动作开始时间的上限
            ...
            {
                double firmUpper = TemporalAnalysis::getActionTSBounds()[a][1].second;//动作结束时间的上限
                if (firmUpper != DBL_MAX) {
                    firmUpper -= stateTS;
    ...
    void extractRP(BuildingPayload * const payload, int & h, list<pair<double, list<ActionSegment> > > & relaxedPlan, pair<int, VAL::time_spec> & earliestTIL, double & makespanEstimate) {
        ...
        payload->fluentLayers.prepareForExtraction();//发出TRPG已经构建完毕，现在要提取解的信号
        
        static const set<int> emptyIntSet;

        RPGRegressionMap goalsAtLayer;

        //遍历目标，将目标实现的时间和完成层记录，并填充goalsAtLayer
        {
            set<int>::iterator gsItr = goals.begin();
        ...
        //若考虑目标的数值信息，在goalsAtLayer对应层填充数值条件
        if (!ignoreNumbers) {
            set<int>::iterator gfItr = goalFluents.begin();
        ...
        //若为时间规划，考虑insistUponEnds，处理目标结束时间。填充goalsAtLayer.actionEnds
        ...
        while (!goalsAtLayer.empty()) {
            ...
            //遍历actionStarts，尝试将其插入relaxedPlan，根据其开始效果、开始前提、数值前提扩展，扩展goalsAtLayer
            {
                ...
                for (; asItr != asEnd; ++asItr) {
                    ...
                    //遍历动作的开始效果，若包含目标的实现，则把该目标从currGAL中删除，并更新tilR取最小值
                    {
                    ...
                    //考虑动作的开始前提，找出其实现层和实现动作，插入goalsAtLayer
                    {
                        ...
                        for (; aplItr != aplEnd; ++aplItr) {
                            ...
                            getAchieverDetails(currPrec, currTS, acIn, acBy);//获取当前命题实现的细节，填充acIn、acBy
            ...
            //遍历actionEnds，尝试将其插入relaxedPlan，根据其结束效果、结束前提、数值结束前提扩展，扩展goalsAtLayer
            {
                ...
                for (; asItr != asEnd; ++asItr) {
                    ...
                    //遍历动作的结束效果，若包含目标实现，更新tilR为最小值，并从currGAL删除该实现的目标
                    {
                    ...
                    //遍历动作的结束前题，访问其实现信息，扩展goalsAtLayer
                    {
                    ...
                    //遍历动作的数值结束前题，根据其实现情况，在goalsAtLayer添加数值条件
                    if (!ignoreNumbers) {
                    ...
                    //将helpful动作插入payload->helpfulActions
                    if (isHelpful) {
            ...
            //根据动作数值条件的实现细节，提取规划解并扩展goalsAtLayer
            if (!ignoreNumbers) {
                ...
                if (!actionsUsed.empty()) {
                    ...
                    for (; sapItr != sapEnd; ++sapItr) {
                        ...
                        //若支撑数值条件的开始动作开始时间小于(currTS - 0.0001)，将其插入goalsAtLayer，然后continue
                        ...
                        //将动作插入松弛规划解，并根据其开始效果删除实现的目标，更具其开始前提/开始数值前提更新goalsAtLayer
                        if (currAchievedBy.second == VAL::E_AT_START) {
                            ...
                            //若当前目标层的目标在支持动作的开始效果中实现，更新tilR并从currGAL中删除该实现目标
                            {
                            ...
                            //处理动作开始前提，访问其实现细节，扩展goalsAtLayer
                            {
                            ...
                            //处理动作数值开始前提，增加payload->fluentLayers的数值条件
                            if (!ignoreNumbers) {
                            ...
                            if (oldR > tilR) oldR = tilR;
                        } else {//将动作插入松弛规划解，并根据其结束效果删除实现的目标，更具其结束前提/结束数值前提更新goalsAtLayer
                            ...
                            //若当前目标层的目标在支持动作的结束效果中实现，更新tilR并从currGAL中删除该实现目标
                            {
                            ...
                            //处理动作结束前提，访问其实现细节，扩展goalsAtLayer。同时，根据动作使用次数addToThePast，判断该动作是否需要调度
                            {
                            ...
                            //处理动作数值结束前提，增加payload->fluentLayers的数值条件
                            if (!ignoreNumbers) {
            ...
            if (!currGAL.empty()) {
                ...
                while (!currGAL.empty()) {
                    ...
                    if (currAchievedBy.second == VAL::E_AT_START) {
                        ...
                        //访问动作开始效果，若实现目标，将该目标从currGAL中删除
                        {
                            list<Literal*> & actionEffectsList = (*actionsToStartEffects)[currAchievedBy.first];
                            list<Literal*>::iterator aelItr = actionEffectsList.begin();
                            const list<Literal*>::iterator aelEnd = actionEffectsList.end();
                        ...
                        //访问动作开始数值前提，添加数值条件。最后还要payload->fluentLayers.recordSideEffects
                        if (!ignoreNumbers) {
                            list<int> & actionPreconditionlist = (*actionsToProcessedStartNumericPreconditions)[currAchievedBy.first];
                            list<int>::iterator aplItr = actionPreconditionlist.begin();
                            const list<int>::iterator aplEnd = actionPreconditionlist.end();
...
vector<vector<set<int> > > RPGHeuristic::Private::actionsAffectedByFluent;//最外层：变量，次外层：0 - 开始， 1 - 结束，最内层：动作ID
...
void RPGHeuristic::doFullExpansion(MinimalState & refState)
{
    ...
    d->buildEmptyActionFluentLookupTable();//构造数据actionsAffectedByFluent
                                 ...
                                 map<double, list<pair<int, int> > > * justApplied, double tilFrom)
{
    ...
    d->integrateContinuousEffects();//根据参数设置确定是否需要将线性效果归结到一起

    //startedActionExtraEndPreconditions.clear();

    d->populateActionFluentLookupTable();//根据动作涉及到的数值（前提/效果/持续时间/线性效果），填充actionsAffectedByFluent

    if (evaluateDebug) cout << "Evaluating a state with " << theState.startedActions.size() << " sorts of actions on the go\n";

    //rpprintState(theState);////输出正文字、已经开始的动作、以及nextTIL
    ...
    d->addRequirementToHaveSeenTheEndOfAllCurrentlyExecutingActions(payload.get());//统计当前TPRG下开始动作中不可以安全跳过的数目
    ...
    d->resetAchievedBy();
    d->recordFactLayerZero(payload.get());//根据系统设置的参数：是否微调RPG，来设定初始状态下各正负fact、数值条件、pne时间
    d->performTILInitialisation();//为可能涉及的文字设置时间期限初值（DBL_MAX或TIL删除时间）

    d->initialiseLatestArrays();////设置earliest/latestStartAllowed、earliest/latestEndAllowed
...

    {
        ...
        d->updateActionsForNewLiteralFact(payload.get(), -1, 0.0, true);//应用无条件动作
    }

    //考虑初始状态的fact，应用前提得到满足的动作并创建对应的事实层
    if (payload->unsatisfiedGoals || payload->unappearedEnds) {
        if (!RPGBuilder::modifiedRPG && RPGBuilder::sortedExpansion) {//按fact实现的调度时间排序，分类扩展，应用以fact为前提的动作
            map<double, list<int> > expansionOrder;//map<fact的调度时间，fact>
        ...
        } else {//考虑初始状态的fact，若该fact的实现在第0层，应用以其为前提的动作，否则创建新的实现层            
    ...
    ////考虑初始状态的数值fact，应用前提得到满足的动作并创建对应的事实层
    if (payload->unsatisfiedGoals || payload->unappearedEnds) {       
        const int loopLim = d->rpgNumericPreconditions->size();

        //考虑初始状态下的数值fact，应用相关动作
        if (loopLim) {
    ...
    //填充逻辑/数值目标层，确定下一层扩展对象nextHappening
    if (payload->unsatisfiedGoals || payload->unappearedEnds) {
        #ifdef POPF3ANALYSIS
        //根据逻辑目标的deadlines填充目标层
    ...
    while (!nextHappening.empty()) {
        ...
        //尝试建立动作应用所需的fluent层，确保将要发生的动作应用之前fluent layer已经建立
        {
        ...
        //若此刻有动作要结束，相应更新并将完成的动作从payload->endActionsAtTime删除
        if (nextHappening.endActionsAppearingAtThisTime != payload->endActionsAtTime.end()) {
            ...
            list<int>::iterator eaItr = eaatItr->second.begin();//当前时刻下要发生的结束动作
        ...
        //若当前层成立的fact在TPRG中已经出现，考虑当前层下所有以其为前提的动作（TIL，Fact等），尝试应用并删除成立的事实层
        if (nextHappening.newFactsAtThisTime != payload->factLayers.end()) {
            ...
            //遍历当前成立的fact，尝试确定以其为前提的动作并应用
            {
                list<int>::iterator stateItr = currFactLayerItr->second.first.begin();//当前事实层的状态
            ...
            //遍历当前成立的代价更低fact，尝试确定以其为前提的动作并应用
            {
            ...
            //遍历当前成立的数值条件，尝试确定以其为前提的动作并应用
            {
                list<int>::iterator stateItr = currFactLayerItr->second.second.begin();//数值条件
            ...
            //遍历当前成立事实层的TIL，尝试更新文字来源，并依情况确定以其为前提的动作并应用
            {
            ...
            //遍历当前事实层的noLongerForbidden动作,若其前提条件均得到满足，尝试更新
            if (currFactLayerItr->second.endOfJustApplied) {
                d->noLongerForbidden.clear();
                set<int> & startSet = currFactLayerItr->second.endOfJustApplied->first;//开始动作
                set<int> & endSet = currFactLayerItr->second.endOfJustApplied->second;//结束动作

                bool spawn = false;

                //遍历刚结束的开始动作，若其在当前的TRPG中forbiddenStart次数为0，则该动作允许开始，同时考虑其前提是否得到满足，若是，插入d->noLongerForbidden
                {
            ...
            //遍历当前成立事实层的负TIL，遍历以其为前提的动作，依情况增加payload->start/end PreconditionCounts
            {
            ...
            //若当前的事实层/数值层已经实现，把它从payload->factLayers中删除
            #ifdef POPF3ANALYSIS
            {
        ...
        if (!failedToMeetDeadline) 
        #endif
        //想要添加的效果均已添加完毕，现在看看是否有新增的数值条件，根据其最早发生时间与下一层EPSILON之后的时间相比较，确定下一个数值扩展层
        {
        // now we've applied all the effects we're going to get, see if new numeric preconditions are true epsilon in the future
            list<int> newNumericPreconditions;            
            payload->fluentLayers.applyRecentlyRecordedEffects(*(d->numericAchievedInLayer), newNumericPreconditions, d->maxNeeded);
            ...
            if (!newNumericPreconditions.empty()) {
                ...
                //排除已经实现的数值目标，unsatisfiedGoals-1
                for (; npItr != npEnd; ++npItr) {
                    if (d->goalFluents.find(*npItr) != d->gfEnd) {
                        --(payload->unsatisfiedGoals);
                    }
                ...
                //遍历新增的数值条件，确定目前各条件成立的最早时间，判断是否在下一层EPSILON之后，添加对应的数值层（下一个要扩展的层）
                if (RPGBuilder::modifiedRPG) {
                    list<int>::const_iterator preID = newNumericPreconditions.begin();
                    const list<int>::const_iterator preIDEnd = newNumericPreconditions.end();
        ...
        //若目标未实现 || 动作未完成 || 目标实现代价大，确定下一层对象nextHappening，否则跳出循环
        if (payload->unsatisfiedGoals || payload->unappearedEnds
            #ifdef POPF3ANALYSIS
    ...
    }//while (!nextHappening.empty())结束

    //若TRPG扩展后目标仍未实现，查看可能的原因并return -1;否则继续程序
    if (payload->unsatisfiedGoals || payload->unappearedEnds
        #ifdef POPF3ANALYSIS
        || payload->tooExpensive
    ...
    //确定rpg数值变量在当前状态下是否满足条件，numericIsTrueInState
    ...
    //根据状态中已经开始的动作和松弛规划解的动作更新h值
    {
        // HACK
        map<int, int> started;
        map<int, int>::iterator insItr = started.end();
        
        //根据状态中已经开始的动作，填充started
        {
            map<int, set<int> >::const_iterator saItr = theState.startedActions.begin();
            const map<int, set<int> >::const_iterator saEnd = theState.startedActions.end();
    ...
    ////在已有helpfulActions的基础上，考虑所有支撑helpfulActions的动作，扩展helpfulActions
    if (RPGBuilder::fullFFHelpfulActions) {
        set<int> startsIn;//AT_START的helpfulactions
        set<int> endsIn;//AT_End的helpfulactions

        //遍历helpfulactions，填充startsIn、endsIn
        {
        ...
        //根据helpfulActions的效果，访问每个效果的支撑动作，考虑将其加入helpfulActions
        {
            list<ActionSegment>::iterator haItr = helpfulActions.begin();
            const list<ActionSegment>::iterator haEnd = helpfulActions.end();
            ...
            for (; haItr != haEnd; ++haItr) {
                ...
                ++haNext;
                const list<Literal*> * effs = 0;//收集动作的开始效果
                if (haItr->second == VAL::E_AT_START) {
                ...
                const double sas = payload->startActionSchedule[haItr->first->getID()];//获取该动作的调度时间

                list<Literal*>::const_iterator eItr = effs->begin();
                const list<Literal*>::const_iterator eEnd = effs->end();

                //遍历帮助动作的开始效果，访问每个效果的支撑动作（effectsToActions），若该动作AT_START且有意义，并没有在startsIn出现过，
                //将其插入helpfulActions
                for (; eItr != eEnd; ++eItr) {
                    ...
                    for (; candItr != candEnd; ++candItr) {
                        set<int> * checkIn = 0;//startsIn，及AT_START的helpfulactions
                        const list<Literal*> * deps = 0;//收集动作candItr的开始前提
                ...
                //遍历动作的结束效果，访问每个效果的支撑动作，若其结束调度时间与帮助动作的结束时间相等，
                //且该支撑动作并未在startsIn和endsIn中出现，将其插入helpfulActions
                if (haItr->second == VAL::E_AT_START) {
                    effs = &((*(d->actionsToEndEffects))[haItr->first->getID()]);//动作的结束效果

                    const double endStamp = payload->endActionSchedule[haItr->first->getID()];//动作的结束时间
                    list<Literal*>::const_iterator eItr = effs->begin();
                    const list<Literal*>::const_iterator eEnd = effs->end();
    ...
    //扩展helpfulActions，haWeights？（没看懂）
...
//遍历动作的开始/结束前提条件（根据第二个参数确定，true：开始，false：结束），确定最小的发生时间
...
//考虑动作的开始/结束添加效果，更新该效果的最优实现层（添加事实层或者更新已有层的动作消耗代价并添加act->fact边）
//只有当目标全满足并且所有动作都完成的时候才返回TRUE
...
bool RPGHeuristic::Private::applyNumericEffects(Private::BuildingPayload * const payload,
                                                ...
                                                const int & limitTo)
{
    const int actualLimit = (limitTo != -1 ? limitTo : RPGBuilder::howManyTimesOptimistic(currAct, payload->startState));//一个变量最多用多少次就会破坏约束
    ...
    //将动作的开始/结束RPG数值效果存入payload->fluentLayers
    {
...
//若第二个参数对应的动作的前提条件均满足，应用相应的效果.只有当所有目标实现并且开始动作都结束的时候才返回TRUE
//第二个参数a的值若为-1：对应无条件动作；若为-2：再也不被禁止的动作，即noLongerForbidden，否则对应processedPreconditionsToActions[a]
//decrement:消耗
bool RPGHeuristic::Private::updateActionsForNewLiteralFact(Private::BuildingPayload * const payload, const int & toPropagate, const double & factLayerTime, const bool & decrementRemainingUnsatisfiedPreconditionCounters)
{
    ...
    list<pair<int, VAL::time_spec> >::iterator depItr = dependents.begin();//无条件动作/可以开始的动作noLongerForbidden/以该文字为前提的动作
    ...
    for (; depItr != depEnd; ++depItr) {
        ...
        if (startAct) {
            ...
            //若不满足的命题前提和数值前提都为0，且动作可以开始，则应用该动作。若应用成功，直接返回TRUE
            if ((!spc && !numericStartPreconditionCounts[currAct]) && forbiddenStart.find(currAct) == forbiddenStart.end() && nlTime < latestStartAllowed[currAct]) {
                ...
                MAKESTARTMDI(POtime, currAct);//POtime = currAct
            ...
            if ((!epc && !numericEndPreconditionCounts[currAct]) && forbiddenEnd.find(currAct) == forbiddenEnd.end() && nlTime < latestEndAllowed[currAct]) {
                ...
                for (int epPass = 0; epPass < 2; ++epPass) {
                    ...
                    if (doLoop) {//factLayerTime >= endActionSchedule[currAct]
                        if (updateDebug) {
                            if (limitToOnce) {
...
//根据动作是否是openEnd，分别应用命题效果、数值效果，更新状态
