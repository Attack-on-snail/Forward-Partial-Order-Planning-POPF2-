...
public:
    ...
    public:
        ...
        int fluentIndex; //*(RPGBuilder::getPNE(fluentIndex))
        VAL::assign_op op; // how to update given fluent index (add to it, assign to it, etc.)
        list<Operand> formula; // formula in postfix后置 notation符号
    ...
    public:
        ...
        double evaluateRHS(vector<double> & fluentTable) const;//计算右值
    ...
    public:
        ...
        vector<int> fluents;//pne的索引，若小于0，-3：？duration; -19: -?duration
        double constant;
        ...
        double evaluate(const vector<double> & fluentTable) {
            ...
            for (int i = 0; i < size; ++i) {
                if (fluents[i] < 0) {
                    return std::numeric_limits<double>::signaling_NaN();//未赋值的变量
                }
    ...
    public:
        ...
        vector<double> weights;//个数为size
        vector<int> variables;//个数为size
        ...
        double evaluate(const vector<double> & maxFluents, const double & minDur, const double & maxDur) const {//计算最大值            
            double toReturn = constant;
            for (int i = 0; i < size; ++i) {
                if (variables[i] >= 0) {
...
// static vector<pair<int, double> > increasedReset;
    ...
    /** @brief  A map from action IDs to a fact that renders（使成为） it uninteresting to apply. */
...
// static void makeWeightedSum(list<Operand> & formula, pair<list<double>, list<int> > & result);
    ...
    static void handleNumericInvariants();//由不变量推断需要新增的前提条件，调用pushInvariantBackThroughStartEffects（）

    static void findSelfMutexes();//确定自我互斥的动作集合selfMutexes

    static void oneShotInferForTILs();//由TIL补充preconditionsToActions集合
    ...
    static void checkConditionalNumericEffectsAreOnlyOnMetricTrackingVariables();//若不是E_METRICTRACKING直接报错，终止程序
     ...
     */
    static void findConcurrentRedundantActions();//没看懂，填充concurrentRedundantActions
    ...
    static void pruneStaticPreconditions();//调用上面的函数

    class CommonRegressionData;

    static void postFilterIrrelevantActions();//过滤那些动作效果与目标实现无关的动作
...
public:
    ...
    static void simplify(pair<list<double>, list<int> > & s);//常值归结到最后一个变量中
    // 数值变量和常量分离，变量在左，常量在右，调用simplify()
    static void makeOneSided(pair<list<double>, list<int> > & LHSvariable, pair<list<double>, list<int> > & RHSvariable, const int & negOffset);
    ...
    /** @return 获取RPG数值前提。A reference to <code>rpgNumericPreconditions</code>, the preconditions used in the problem, in LNF. */
...
public:
     ...
     */
    static void setGuidance(const char * config);//命令行-g 配置启发式
