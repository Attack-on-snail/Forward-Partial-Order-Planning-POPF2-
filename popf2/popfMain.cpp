...
int main(int argc, char * argv[])
{
    ...
    performTIMAnalysis(&argv[argcount]); /* 检查输入的领域文件和问题文件是否有词法、语法、语义等错误 */
    ...
    RPGBuilder::initialise();//操作实例化、过滤静态谓词和不相关的动作
    ...
    if (readInAPlan) {
        ...
        planAndConstraints = FF::search(reachesGoals);//搜索
        cout<<endl;
        FF::printPlanAsDot(cout, *planAndConstraints.plan, planAndConstraints.constraints);
    }
    ...
    */
        ...
        if (RPGBuilder::getRPGDEs(ID).empty()) {// non-durative action
            ...
            */
            for (int pass = 0; pass < 2; ++pass) {
                ...
                toInsert.minDuration  = actDur;//增加
    ...
    if (debug) {
        ...
        for (int i = 0; insItr != insEnd; ++insItr, ++i) {
            cout << i << ": ";
            if (insItr->action) {
                cout << (*insItr).lpTimestamp<<" :";//新增
                cout << *(insItr->action);
                cout <<" ["<<(*insItr).minDuration<<"] ";//新增
