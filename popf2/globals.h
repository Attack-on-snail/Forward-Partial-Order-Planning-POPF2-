...
typedef set<Literal*, LiteralLT> LiteralSet;/* LiteralLT：重载排序函数，根据ID从小到大排列 */


/** @brief Class describing a snap-action. 分段动作：开始、结束、时间初始化（虚动作） */
class ActionSegment
{
...
public:
    ...
    /** @brief Step IDs of actions that must have finished prior to this one.  Deprecated（强烈反对）. */
