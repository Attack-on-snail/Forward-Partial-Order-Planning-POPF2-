                                                      ...
                                                      pair<const RPGBuilder::RPGNumericPrecondition *, bool> & preResult, pair<RPGBuilder::ArtificialVariable *, bool> & avResult)
{
    ...
    map<int, double> lhs;//变量序号，常量（正值、负值、权重）
...
void RPGBuilder::handleNumericInvariants()
{
    ...
    // 删除processedRPGNumericPreconditionsToActions、rpgNumericPreconditionsToActions的中间动作over-all
    {
...
//  actionsToProcessedStartNumericPreconditions = vector<list<NumericPrecondition> >(opCount);
    ...
    for (int i = 0; i < opCount; ++i) {
        ...
        if (!rogueActions[i]) {
            ...
            //根据不变量数值条件倒退动作开始的时候是否需要增加新的前提条件，填充localAVs，localPres
...
//      negativeNumericAchievedByReset.resize(newPreLimit);
    ...
    //填充processedRPGNumericPreconditionsToActions的动作，AT_START
...
/*
    currPreList : 当前的数值前提条件
    destList    ：前提编号
    toIncrement ： 更新后的数值前提数量
    */
...
void RPGBuilder::buildRPGNumericPreconditions()
{
    ...
    for (int i = 0; i < opCount; ++i) {
        ...
        //构建RPG数值前提并实现常变量分离，分离后变量在左，常量在右
