...
public :
    ...
    symbol_class* symbol_ref(const string& name)
	{
        ...
        _Base::insert(std::make_pair(name,sym));
		return sym;
	    }
	};
    ...
    void set_either_types(typed_symbol_list<pddl_type>* tl)
 	{
 	    ...
 	    for (; i!=_Base::end(); ++i)
 	    {
            (*i)->either_types= new typed_symbol_list<pddl_type>(*tl);//原来没有pddl_type报错才加上的
 		}
 	};
...
{
private:
    polarity plrty;    // +ve or -ve goals //polarity:极性，正级、负级
    proposition* prop;
...
{
private:
    const quantifier qfier;//存在量词、全称量词
